...
def size_with_scop() -> float:
    ...
    print(f'When sizing with a constant SCOP, the required borehole depth is {depth:.2f}m')
...
def size_with_part_load_data():
    ...
    """
    seer = SEER(1000)
    cop = COP(np.array([[4.42, 5.21, 6.04, 7.52, 9.5],
                        [3.99, 4.58, 5.21, 6.02, 6.83],
                        [3.86, 4.39, 4.97, 5.62, 6.19],
                        [3.8, 4.3, 4.86, 5.44, 5.9],
                        [3.76, 4.25, 4.79, 5.34, 5.74]]),
              np.array([-5, 0, 5, 10, 15]),
              range_part_load=np.array([0, 25, 50, 75, 100]))
