...
def load_data_GUI(
    filename: str, thermal_demand: int, heating_load_column: str, cooling_load_column: str, combined: str, sep: str, dec: str, fac: float, hourly: bool = False
):
    """
...
class GUI(GuiStructure):
    ...
    def _page_options(self, translations):
        ...
        self.option_method_size_depth = ButtonBox(
            label=translations.option_method_size_depth, default_index=0, entries=[" L2 ", " L3 ", "  L4  "], category=self.category_calculation
        )
        self.option_method_rb_calc = ButtonBox(
            label=translations.option_method_rb_calc, default_index=1, entries=[" constant ", " dynamic "], category=self.category_calculation
        )
        self.option_temperature_profile_hourly = ButtonBox(
            label=translations.option_temperature_profile_hourly, default_index=0, entries=[" no ", " yes "], category=self.category_calculation
        )

        self.category_advanced_options = Category(page=self.page_options, label=translations.category_advanced_options)
        self.option_atol = FloatBox(
            translations.option_atol, 0.05, self.category_advanced_options, decimal_number=2, minimal_value=0.01, maximal_value=100, step=0.05
        )
        self.option_rtol = FloatBox(
            translations.option_rtol, 0.5, self.category_advanced_options, decimal_number=1, minimal_value=0.1, maximal_value=100, step=1
        )
        self.option_max_nb_of_iter = IntBox(translations.option_max_nb_of_iter, 40, self.category_advanced_options, minimal_value=1, maximal_value=100, step=1)
    ...
    def _page_borefield(self, translations):
        ...
        self.option_borefield_radius = els.FloatBox(
            category=self.category_borefield,
            label=translations.option_borefield_radius,
            minimal_value=0,
            maximal_value=1_000_000,
            default_value=12.5,
            step=0.1,
            decimal_number=2,
        )
        ...
        self.option_number_circle_boreholes = els.IntBox(
            category=self.category_borefield,
            label=translations.option_number_circle_boreholes,
            minimal_value=2,
            maximal_value=1_000_000,
            default_value=12,
            step=1,
        ...
        self.option_width = IntBox(category=self.category_borefield, label=translations.option_width, default_value=9, minimal_value=1, maximal_value=40)
        self.option_width.change_event(self.update_borefield)
        self.option_length = IntBox(category=self.category_borefield, label=translations.option_length, default_value=12, minimal_value=1, maximal_value=40)
        ...
        self.option_seperator_borefield = ButtonBox(
            label=translations.option_seperator_borefield,
            default_index=0,
            entries=['Semicolon ";"', 'Comma ","', 'Tab "   "'],
            category=self.category_borefield,
        )
        self.option_decimal_borefield = ButtonBox(
            label=translations.option_decimal_borefield, default_index=0, entries=['Point "."', 'Comma ","'], category=self.category_borefield
        )

        file = f"{FOLDER.joinpath('gui/test_gui/borefield_data.csv')}"
        self.borefield_file = els.FileNameBox(
            label=translations.borefield_file, category=self.category_borefield, default_value=file, file_extension=["csv", "txt"]
        )
        ...
        self.custom_borefield = els.FlexibleAmount(
            label=translations.custom_borefield, default_length=1, entry_mame="Borehole", category=self.category_borefield, min_length=1
        )
        ...
        self.custom_borefield.add_option(
            els.FloatBox, name="Borehole radius [m]", default_value=0.075, minimal_value=0, maximal_value=1_000, decimal_number=4, step=0.01
        )
        ...
        _ = [
            aim.change_event(
                partial(
                    ...
                    [self.aim_optimize, self.aim_temp_profile, self.aim_req_depth],
                    [self.aim_rect, self.aim_Box_shaped, self.aim_L_shaped, self.aim_U_shaped, self.aim_circle],
                    self.option_pipe_borehole_radius,
                )
            )
            for aim in li_aim
        ]
        ...
        _ = [
            aim.change_event(
                partial(
                    ...
                    [self.aim_optimize, self.aim_temp_profile, self.aim_req_depth],
                    [self.aim_circle],
                    self.option_number_circle_boreholes,
                )
            )
            for aim in li_aim
        ]
        ...
        _ = [
            aim.change_event(
                partial(show_option_on_multiple_aims, [self.aim_optimize, self.aim_temp_profile, self.aim_req_depth], [self.aim_custom], self.custom_borefield)
            )
            for aim in li_aim
        ]
        ...
        _ = [
            aim.change_event(
                partial(show_option_on_multiple_aims, [self.aim_optimize, self.aim_temp_profile, self.aim_req_depth], [self.aim_custom], self.import_borefield)
            )
            for aim in li_aim
        ]
        ...
        _ = [
            aim.change_event(
                partial(show_option_on_multiple_aims, [self.aim_optimize, self.aim_temp_profile, self.aim_req_depth], [self.aim_custom], self.borefield_file)
            )
            for aim in li_aim
        ]
        ...
        _ = [
            aim.change_event(
                partial(
                    ...
                    [self.aim_optimize, self.aim_temp_profile, self.aim_req_depth],
                    [self.aim_custom],
                    self.option_seperator_borefield,
                )
            )
            for aim in li_aim
        ]
    ...
    def _page_earth(self, translations):
        ...
        self.option_source_ground_temperature = ButtonBox(
            category=self.category_earth, label=translations.option_source_ground_temperature, default_index=1, entries=["Measured", "Database", "Custom"]
        )
        self.option_flux_gradient = ButtonBox(
            category=self.category_earth, label=translations.option_flux_gradient, default_index=0, entries=["Flux", "Gradient"]
        )

        self.option_ground_database = ListBox(
            category=self.category_earth, label=translations.option_ground_database, default_index=0, entries=LIST_OF_COUNTRY_NAMES
        )
        self.option_ground_database.change_event(self._update_selected_ground_temperature_from_database)

        self.hint_ground_database = ResultText(
            translations.hint_ground_database, category=self.category_earth, prefix="The ground temperature at the selected location is: ", suffix="°C"
        )
        self.hint_ground_flux_database = ResultText(
            translations.hint_ground_flux_database, category=self.category_earth, prefix="The geothermal heat flux at the selected location is: ", suffix="W/m²"
        )
        ...
        self.option_len_peak_heating = FloatBox(
            category=self.category_temperatures,
            label=translations.option_len_peak_heating,
            default_value=6,
            minimal_value=1,
            maximal_value=8760,
            step=1,
        ...
        self.option_len_peak_cooling = FloatBox(
            category=self.category_temperatures,
            label=translations.option_len_peak_cooling,
            default_value=6,
            minimal_value=1,
            maximal_value=8760,
            step=1,
    ...
    def _page_borehole_resistance(self, translations):
        ...
        self.option_fluid_selector = ButtonBox(
            category=self.category_fluid_data, label=translations.option_fluid_selector, default_index=0, entries=["Custom", "Glycol mixture"]
        )

        self.option_glycol_selector = ButtonBox(
            category=self.category_fluid_data, label=translations.option_glycol_selector, default_index=0, entries=["Ethylene glycol", "Propylene glycol"]
        )
        ...
        self.option_fluid_ref_temp = FloatBox(
            category=self.category_fluid_data, label=translations.option_fluid_ref_temp, decimal_number=1, default_value=10, maximal_value=100, minimal_value=0
        )
        ...
        self.option_U_pipe_or_coaxial_pipe = ButtonBox(
            category=self.category_pipe_data, label=translations.option_U_pipe_or_coaxial_pipe, default_index=0, entries=["U-pipe", "Coaxial pipe"]
        )
        ...
        self.option_pipe_coaxial_inner_outer.change_event(
            lambda: self.option_pipe_coaxial_inner_inner.widget.setMaximum(self.option_pipe_coaxial_inner_outer.get_value())
        )
        self.option_pipe_coaxial_inner_inner.change_event(
            lambda: self.option_pipe_coaxial_inner_outer.widget.setMinimum(self.option_pipe_coaxial_inner_inner.get_value())
        )
        ...
        self.option_pipe_coaxial_inner_outer.change_event(
            lambda: self.option_pipe_coaxial_outer_inner.widget.setMinimum(self.option_pipe_coaxial_inner_outer.get_value())
        )
        ...
        self.pipe_thermal_resistance = ResultText(
            translations.pipe_thermal_resistance,
            category=self.category_pipe_data,
            prefix="The equivalent borehole thermal resistance (at 150m) is: ",
            suffix="mK/W",
        )
    ...
    def _page_thermal_demand(self, translations):
        ...
        def create_category_select_datafile():
            ...
            self.option_seperator_csv = ButtonBox(
                label=translations.option_seperator_csv, default_index=0, entries=['Semicolon ";"', 'Comma ","'], category=self.category_select_file
            )
            self.option_decimal_csv = ButtonBox(
                label=translations.option_decimal_csv, default_index=0, entries=['Point "."', 'Comma ","'], category=self.category_select_file
            )
            ...
            self.option_column = ButtonBox(
                label=translations.option_column, default_index=0, entries=["1 column", "2 columns"], category=self.category_select_file
            )
            ...
            self.option_unit_data = ButtonBox(
                label=translations.option_unit_data, default_index=1, entries=["W", "kW", "MW"], category=self.category_select_file
            )

            self.hint_press_load = Hint(hint=translations.hint_press_load, category=self.category_select_file, warning=True)
        ...
        def create_category_building_demand():
            self.category_demand_building_or_geo = Category(page=self.page_thermal, label=translations.category_demand_building_or_geo)
            self.geo_load = ButtonBox(
                label=translations.geo_load, default_index=0, entries=[" geothermal ", " building "], category=self.category_demand_building_or_geo
            )
            self.option_include_dhw = ButtonBox(
                label=translations.option_include_dhw, default_index=0, entries=[" No ", " Yes "], category=self.category_demand_building_or_geo
            )
            self.SCOP = FloatBox(
    ...
    def _page_results(self, translations):
        ...
        def create_category_numerical_results():
            ...
            self.result_Rb_calculated = ResultText(
                translations.result_Rb_calculated, category=self.numerical_results, prefix="Equivalent borehole thermal resistance: ", suffix="Wm/K"
            )
            ...
            self.result_Reynolds = ResultText(translations.result_Reynolds, category=self.numerical_results, prefix="Reynolds number: ", suffix="")
            self.result_Reynolds.text_to_be_shown("Borefield", "Re")
            self.result_Reynolds.function_to_convert_to_text(lambda x: round(x, 0))

            self.results_ground_temperature = ResultText(
                translations.results_ground_temperature, category=self.numerical_results, prefix="Average ground temperature: ", suffix=" deg C"
            )
            ...
            self.results_heating_load = ResultText(
                translations.results_heating_load, category=self.numerical_results, prefix="Heating load on the borefield: ", suffix=" kWh"
            )
            self.results_heating_peak_geo = ResultText(
                translations.results_heating_peak_geo, category=self.numerical_results, prefix="with a peak of: ", suffix=" kW"
            )
            ...
            self.results_heating_load_percentage = ResultText(
                translations.results_heating_load_percentage, category=self.numerical_results, prefix="This is ", suffix="% of the heating load"
            )
            ...
            self.results_heating_ext = ResultText(
                translations.results_heating_ext, category=self.numerical_results, prefix="heating load external: ", suffix=" kWh"
            )
            ...
            self.results_heating_peak = ResultText(translations.results_heating_peak, category=self.numerical_results, prefix="with a peak of: ", suffix=" kW")
            ...
            self.results_cooling_load = ResultText(
                translations.results_cooling_load, category=self.numerical_results, prefix="Cooling load on the borefield: ", suffix=" kWh"
            )
            self.results_cooling_peak_geo = ResultText(
                translations.results_cooling_peak_geo, category=self.numerical_results, prefix="with a peak of: ", suffix=" kW"
            )
            ...
            self.results_cooling_load_percentage = ResultText(
                translations.results_cooling_load_percentage, category=self.numerical_results, prefix="This is ", suffix="% of the cooling load"
            )
            ...
            self.results_cooling_ext = ResultText(
                translations.results_cooling_ext, category=self.numerical_results, prefix="cooling load external: ", suffix=" kWh"
            )
            ...
            self.results_cooling_peak = ResultText(translations.results_cooling_peak, category=self.numerical_results, prefix="with a peak of: ", suffix=" kW")
            ...
            self.max_temp = ResultText(
                translations.max_temp, category=self.numerical_results, prefix="The maximum average fluid temperature is ", suffix=" deg C"
            )
            ...
            self.min_temp = ResultText(
                translations.min_temp, category=self.numerical_results, prefix="The minimum average fluid temperature is ", suffix=" deg C"
            )
        ...
        def create_figure_temperature_profile():
            self.figure_temperature_profile = ResultFigure(label=translations.figure_temperature_profile, page=self.page_result)

            self.figure_temperature_profile.fig_to_be_shown(class_name="Borefield", function_name="print_temperature_profile")

            self.legend_figure_temperature_profile = FigureOption(
                category=self.figure_temperature_profile,
            ...
            self.hourly_figure_temperature_profile = FigureOption(
                category=self.figure_temperature_profile,
        ...
        def create_figure_load_duration():
            self.figure_load_duration = ResultFigure(label=translations.figure_load_duration, page=self.page_result)

            self.figure_load_duration.fig_to_be_shown(class_name="Borefield", function_name="plot_load_duration")

            self.legend_figure_load_duration = FigureOption(
                category=self.figure_load_duration,
    ...
    def _page_settings(self, translations):
        ...
        self.option_advanced_options = ButtonBox(translations.option_advanced_options, 0, [" No ", " Yes "], self.category_save_scenario)
    ...
    def set_dependencies(self) -> None:
        self.show_option_under_multiple_conditions(self.option_method_size_depth, self.aim_req_depth, custom_logic=self.aim_req_depth.is_checked)
        self.show_option_under_multiple_conditions(self.option_temperature_profile_hourly, self.aim_temp_profile, custom_logic=self.aim_temp_profile.is_checked)

        self.show_option_under_multiple_conditions(
            self.option_ground_temp, self.option_source_ground_temperature, custom_logic=partial(self.option_source_ground_temperature.check_linked_value, 0)
        )
        self.show_option_under_multiple_conditions(
            [self.hint_ground_database, self.hint_ground_flux_database, self.option_ground_database],
            self.option_source_ground_temperature,
            custom_logic=partial(self.option_source_ground_temperature.check_linked_value, 1),
        )
        self.show_option_under_multiple_conditions(
            [self.option_flux_gradient, self.option_ground_temp_gradient],
            self.option_source_ground_temperature,
            custom_logic=partial(self.option_source_ground_temperature.check_linked_value, 2),
        )
        self.show_option_under_multiple_conditions(
            self.option_ground_heat_flux,
            self.option_flux_gradient,
            custom_logic=partial(self.option_flux_gradient.check_linked_value, 0),
            check_on_visibility_change=True,
        )
        self.show_option_under_multiple_conditions(
            self.option_temp_gradient,
            self.option_flux_gradient,
            custom_logic=partial(self.option_flux_gradient.check_linked_value, 1),
            check_on_visibility_change=True,
        )

        self.show_option_under_multiple_conditions(
            [self.option_len_peak_heating, self.option_len_peak_cooling],
            [self.aim_optimize, self.option_temperature_profile_hourly, self.option_method_size_depth],
        ...
        self.show_option_under_multiple_conditions(
            self.category_constant_rb, self.option_method_rb_calc, custom_logic=partial(self.option_method_rb_calc.check_linked_value, 0)
        )
        self.show_option_under_multiple_conditions(
            [self.category_fluid_data, self.category_pipe_data],
            self.option_method_rb_calc,
            custom_logic=partial(self.option_method_rb_calc.check_linked_value, 1),
        )

        self.show_option_under_multiple_conditions(
            [self.option_fluid_capacity, self.option_fluid_conductivity, self.option_fluid_density, self.option_fluid_viscosity],
        ...
        self.show_option_under_multiple_conditions(
            [self.option_glycol_selector, self.option_glycol_percentage, self.option_fluid_ref_temp],
            [self.option_fluid_selector, self.option_method_rb_calc],
        ...
        self.show_option_under_multiple_conditions(
            [
                self.option_pipe_coaxial_inner_inner,
        ...
        self.show_option_under_multiple_conditions(
            [self.option_pipe_inner_radius, self.option_pipe_outer_radius, self.option_pipe_number, self.option_pipe_distance],
            self.option_U_pipe_or_coaxial_pipe,
            custom_logic=partial(self.option_U_pipe_or_coaxial_pipe.check_linked_value, 0),
            check_on_visibility_change=True,
        )

        self.show_option_under_multiple_conditions(
            [self.option_heating_column, self.option_cooling_column], self.option_column, custom_logic=partial(self.option_column.check_linked_value, 1)
        )
        self.show_option_under_multiple_conditions(
            self.option_single_column, self.option_column, custom_logic=partial(self.option_column.check_linked_value, 0)
        )

        self.show_option_under_multiple_conditions(
            [self.button_load_csv, self.hint_press_load, self.category_th_demand],
            [self.option_method_size_depth, self.option_temperature_profile_hourly],
            functions_check_for_or=[
                partial(self.option_method_size_depth.check_linked_value, 0),
                partial(self.option_method_size_depth.check_linked_value, 1),
                partial(self.option_temperature_profile_hourly.check_linked_value, 0),
        ...
        self.show_option_under_multiple_conditions([self.SCOP, self.SEER], self.geo_load, custom_logic=partial(self.geo_load.check_linked_value, 1))

        self.show_option_under_multiple_conditions(self.option_include_dhw, self.aim_optimize, custom_logic=lambda: not self.aim_optimize.is_checked())
        self.show_option_under_multiple_conditions(
            self.DHW, self.option_include_dhw, custom_logic=partial(self.option_include_dhw.check_linked_value, 1), check_on_visibility_change=True
        )

        self.show_option_under_multiple_conditions(
            self.SCOP_DHW,
            [self.geo_load, self.option_include_dhw],
            functions_check_for_and=[partial(self.geo_load.check_linked_value, 1), partial(self.option_include_dhw.check_linked_value, 1)],
            check_on_visibility_change=True,
        )
        ...
        list_options_optimize_load_profile = [
            self.results_heating_ext,
            self.results_heating_peak_geo,
            self.results_heating_load_percentage,
            self.results_heating_load,
            self.results_heating_peak,
            self.results_cooling_ext,
            self.results_cooling_load_percentage,
            self.results_cooling_load,
            self.results_cooling_peak,
            self.results_cooling_peak_geo,
        ]
        self.show_option_under_multiple_conditions(
            list_options_optimize_load_profile, self.aim_optimize, custom_logic=self.aim_optimize.is_checked, check_on_visibility_change=True
        )

        self.show_option_under_multiple_conditions(
            self.results_ground_temperature,
            self.option_source_ground_temperature,
            functions_check_for_or=[
        ...
        self.show_option_under_multiple_conditions(
            [self.result_Rb_calculated, self.result_Reynolds],
            self.option_method_rb_calc,
            custom_logic=partial(self.option_method_rb_calc.check_linked_value, 1),
            check_on_visibility_change=True,
        )
        ...
        self.show_option_under_multiple_conditions(
            self.result_text_depth, self.aim_req_depth, custom_logic=partial(self.aim_req_depth.is_checked), check_on_visibility_change=True
        )

        self.show_option_under_multiple_conditions(
            [self.max_temp, self.min_temp], self.aim_temp_profile, custom_logic=partial(self.aim_temp_profile.is_checked), check_on_visibility_change=True
        )

        self.show_option_under_multiple_conditions(
            [self.hourly_figure_temperature_profile, self.figure_load_duration],
            [self.option_temperature_profile_hourly, self.aim_optimize, self.option_method_size_depth],
            functions_check_for_or=[
    ...
    def check_correct_pipe_data(self) -> None:
        ...
        def Upipe():
            ...
            self.option_pipe_borehole_radius_2.widget.setMinimum(max(r_outer_pipe + distance_min, self.option_pipe_borehole_radius_2.minimal_value))
            self.option_pipe_borehole_radius_2.widget.blockSignals(False)
            self.option_pipe_borehole_radius.widget.blockSignals(True)
            self.option_pipe_borehole_radius.widget.setMinimum(max(r_outer_pipe + distance_min, self.option_pipe_borehole_radius.minimal_value))
            ...
            self.option_pipe_outer_radius.widget.setMaximum(min(r_borehole - distance_min, self.option_pipe_outer_radius.maximal_value))
        ...
        def coaxial():
            ...
            self.option_pipe_borehole_radius_2.widget.blockSignals(True)
            self.option_pipe_borehole_radius_2.widget.setMinimum(max(r_outer_pipe, self.option_pipe_borehole_radius_2.minimal_value))
            self.option_pipe_borehole_radius_2.widget.blockSignals(False)
            self.option_pipe_borehole_radius.widget.blockSignals(True)
            self.option_pipe_borehole_radius.widget.setMinimum(max(r_outer_pipe, self.option_pipe_borehole_radius.minimal_value))
            self.option_pipe_borehole_radius.widget.blockSignals(False)
    ...
    def update_borehole(self) -> None:
        ...
        None
        """
        frame = self.category_pipe_data.graphic_left if self.category_pipe_data.graphic_left is not None else self.category_pipe_data.graphic_right
    ...
    def update_borefield(self) -> None:
        ...
        def draw_borefield():
            ...
            if self.aim_rect.widget.isChecked():
                coordinates = [
                    (w * spacing_width * scale - width, l * spacing_length * scale - length)
                ...
                coordinates = [
                    (w * spacing_width * scale - width, l * spacing_length * scale - length)
                    for w in range(self.option_width.get_value())
                    if not (0 < w < self.option_width.get_value() - 1)
                    for l in range(self.option_length.get_value())
                ]
                coordinates += [
                    (w * spacing_width * scale - width, l * spacing_length * scale - length)
                    for w in range(self.option_width.get_value())
                    for l in range(self.option_length.get_value())
                    if not (0 < l < self.option_length.get_value() - 1)
                ]
                ...
                coordinates = [
                    (sin(angle * n) * radius - r_bore * scale / 2, cos(angle * n) * radius - r_bore * scale / 2)
                    for n in range(self.option_number_circle_boreholes.get_value())
                ]
    ...
    def update_borehole_thermal_resistance(self) -> None:
        ...
        try:
            fluid_data = FluidData(
                self.option_fluid_mass_flow.get_value(),
                self.option_fluid_conductivity.get_value(),
                self.option_fluid_density.get_value(),
            ...
            if self.option_fluid_selector.get_value() == 1:
                ...
                if self.option_glycol_selector.get_value() == 0:
                    fluid_data.import_fluid_from_pygfunction(
                        gt.media.Fluid("MEG", self.option_glycol_percentage.get_value(), self.option_fluid_ref_temp.get_value())
                    )
                else:
                    fluid_data.import_fluid_from_pygfunction(
                        gt.media.Fluid("MPG", self.option_glycol_percentage.get_value(), self.option_fluid_ref_temp.get_value())
                    )
            if self.option_U_pipe_or_coaxial_pipe.get_value() == 0:
                pipe_data = MultipleUTube(
                    self.option_pipe_grout_conductivity.get_value(),
                    self.option_pipe_inner_radius.get_value(),
                    self.option_pipe_outer_radius.get_value(),
                    self.option_pipe_conductivity.get_value(),
                    self.option_pipe_distance.get_value(),
                    self.option_pipe_number.get_value(),
                    self.option_pipe_roughness.get_value(),
                )
            else:
                pipe_data = CoaxialPipe(
                    ...
                    self.option_pipe_roughness.get_value(),
                    True,
                )  # constant since this does not change anything
            borehole = Borehole(fluid_data, pipe_data)
            resistance = borehole.get_Rb(
                self.option_depth.get_value(),
                self.option_pipe_depth.get_value(),
                self.option_pipe_borehole_radius.get_value(),
