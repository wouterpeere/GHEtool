...
def test_size_inlet_outlet_l3():
    ...
    borefield.flow_data = ConstantFlowRate(mfr=1)
    borefield.pipe_data = pipeData
    borefield.calculation_setup(use_constant_Rb=False, interpolate_gfunctions=False)

    # extraction
    borefield.calculation_setup(size_based_on='inlet')
    size_inlet = borefield.size_L3(20)
    borefield.calculation_setup(size_based_on='outlet')
    size_outlet = borefield.size_L3(20)
    borefield.calculation_setup(size_based_on='average')
    delta_T = load.peak_extraction / (
                borefield.fluid_data.cp() / 1000 * borefield.flow_data.mfr() * borefield.number_of_boreholes)
    borefield.set_min_avg_fluid_temperature(delta_T / 2)
    assert np.isclose(size_inlet, borefield.size_L3(20))
    borefield.set_min_avg_fluid_temperature(-delta_T / 2)
    assert np.isclose(size_outlet, borefield.size_L3(20))
