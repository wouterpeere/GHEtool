...
from GHEtool.VariableClasses import MonthlyBuildingLoadAbsolute, HourlyGeothermalLoad, MonthlyGeothermalLoadMultiYear
...
def test_imbalance():
    load = MonthlyBuildingLoadAbsolute(np.ones(12) * 10, np.ones(12), np.ones(12), np.ones(12))
    assert load.imbalance == -108
    load = MonthlyBuildingLoadAbsolute(np.ones(12), np.ones(12) * 10, np.ones(12), np.ones(12))
    assert load.imbalance == 108
...
def test_params_last_year():
    load = MonthlyBuildingLoadAbsolute(*load_case(2))
...
def test_dummy_peak_duration():
    test = MonthlyBuildingLoadAbsolute()
...
def test_dhw():
    ...
    assert np.array_equal(
        np.array([7440., 6720., 7440., 7200., 7440., 7200., 7440., 7440., 7200., 7440., 7200., 7440.]),
        load.baseload_heating)
...
def test_eq():
    load_1 = MonthlyBuildingLoadAbsolute(*load_case(2))
    load_2 = MonthlyBuildingLoadAbsolute(*load_case(2))
...
def test_add():
    load_1 = MonthlyBuildingLoadAbsolute(*load_case(2))
    load_2 = MonthlyBuildingLoadAbsolute(*load_case(1))
    load_1.dhw = 10000
    load_2.dhw = 50000
    ...
    assert np.allclose(result._baseload_heating,
